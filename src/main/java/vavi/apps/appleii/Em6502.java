...
public class Em6502 {
     ...
     */
    private int zeroPageRead(int addr) {
        return mem[addr] & 0xff;
    }

    private void zeroPageWrite(int addr, int value) {
        mem[addr] = (byte) value;
    }
    ...
    private final int[] BCDTableSub;    // subtraction correction
...
//		createRunFile(); // TODO: for debugging - disable
    ...
    private boolean getFN() {
        return ((NZFlags & 0x280) != 0);
    }

    private boolean getFNotN() {
        return ((NZFlags & 0x280) == 0);
    }

    private boolean getFZ() {
        return ((NZFlags & 0xff) == 0);
    }

    private boolean getFNotZ() {
        return ((NZFlags & 0xff) != 0);
    }

    private void setFNZ(boolean n, boolean z) {
        NZFlags = ((n) ? 0x200 : 0x00) | ((z) ? 0x00 : 0x01);
    }
     ...
     */
    @SuppressWarnings("unused")
    private void checkCrossPage(int addr, int offset) {
    ...
    private int eazpx() {
        easp1 = (memoryRead(PC) + X) & 0xff;
        PC++;
        return easp1;
    }

    private int eazpy() {
        easp1 = (memoryRead(PC) + Y) & 0xff;
        PC++;
        return easp1;
    }
...
//		return ((easp1 & 0x80) != 0) ? easp1 - 256 : easp1;
     ...
     * Misc. macros
     */
    private void adcBCDAdjust() {
        if (getD()) result = BCDTableAdd[result];
    }

    private void sbcBCDAdjust() {
