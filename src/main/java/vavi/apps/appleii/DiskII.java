...
public class DiskII extends Peripheral {
    ...
    private final byte[][][] disk = new byte[NUM_DRIVES][DOS_NUM_TRACKS][];
...
//	  private int[] gcrDecodingTable = new int[256];
    ...
    public void readDisk(AppleIIGo.Dao dao, int drive, String resource, int volume, boolean isWriteProtected) {
        ...
        if (resource != null) {
            ...
            if (resource.toLowerCase().endsWith(".nib")) {
                logger.log(Level.TRACE, "DRIVE[" + drive + "]: NIB");
                isNib = true;
            } else {
                logger.log(Level.TRACE, "DRIVE[" + drive + "]: DSK");
            }
        }
     ...
     */
    private void writeSync(int length) {
        while (length > 0) {
            length--;
            gcrWriteNibble(0xff);
        }
